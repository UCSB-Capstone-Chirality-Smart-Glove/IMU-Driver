...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomHndataHdle;                    /**< handData handle */
 ...
 */
#define COPY_HANDDATA_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
#define COPY_FINGERTWO_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x10,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_FINGERTHREE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x20,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_FINGERFOUR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x30,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_THUMBDATA_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x40,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for handData +
   *                                2 for fingerOne +
   *                                2 for fingerTwo +
   *                                2 for fingerThree +
   *                                2 for fingerFour +
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomHndataHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: hnData, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: hnData \n\r");
  }
