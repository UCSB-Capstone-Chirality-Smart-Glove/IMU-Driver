...
void read_sensor(struct bmi3_dev dev, float data[], uint16_t dataI[])
{
	uint8_t acc_regs[6];
	uint8_t gyr_regs[6];
	uint8_t rslt;
	...
	dataI[0] = (gyr_regs[0] | (uint16_t)gyr_regs[1] << 8);
	dataI[1] = (gyr_regs[2] | (uint16_t)gyr_regs[3] << 8);
	dataI[2] = (gyr_regs[4] | (uint16_t)gyr_regs[5] << 8);
	...
	dataI[3] = (acc_regs[0] | (uint16_t)acc_regs[1] << 8);
	dataI[4] = (acc_regs[2] | (uint16_t)acc_regs[3] << 8);
	dataI[5] = (acc_regs[4] | (uint16_t)acc_regs[5] << 8);

	data[0] = lsb_to_dps(dataI[0], (float)2000, dev.resolution);
	data[1] = lsb_to_dps(dataI[1], (float)2000, dev.resolution);
	data[2] = lsb_to_dps(dataI[2], (float)2000, dev.resolution);

	data[3] = lsb_to_mps2(dataI[3], 2, dev.resolution);
	data[4] = lsb_to_mps2(dataI[4], 2, dev.resolution);
	data[5] = lsb_to_mps2(dataI[5], 2, dev.resolution);
