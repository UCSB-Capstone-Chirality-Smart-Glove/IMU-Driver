...
rotation_vec3 matrix_to_euler(vec3 matrix[3]) {
	float r31 = matrix[2].x;
	if (r31 == 1 || r31 == -1) {
		float yaw = 0;
		float pitch = 90 * r31;
		float roll = yaw * r31 + atan2(r31 * matrix[0].y, r31 * matrix[0].z) * (180.0 / M_PI);
		return (rotation_vec3) {roll, pitch, yaw};
	}
	else {
		float roll = atan(-matrix[2].y / matrix[2].z) * (180.0 / M_PI);
		float pitch = asin(matrix[2].x) * (180.0 / M_PI);
		float yaw = atan(-matrix[1].x / matrix[0].x) * (180.0 / M_PI);
		return (rotation_vec3) {roll, pitch, yaw};
	}
}
